...
<body class="main-layout inner_page">
    ...
    <div class="pagination">
        <?php
        $query = "SELECT book_count as total FROM tblbook";
        $result = $con->query($query);
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            echo "<li class='pagination-item'><a href='?page=$i";
            if (!empty($filtervalues)) {
                echo "&search=$filtervalues";
            }
            if (!empty($sort_option)) {
                echo "&sort_alphabet=$sort_option";
            }
            echo "&result_count=$resultsPerPage'>$i</a></li>";
        }
        echo "</ul>";
        ?>
    </div>
